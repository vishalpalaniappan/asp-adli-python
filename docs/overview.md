...
This design realizes a library manager that provides the user with a menu to interact with it. The user can submit books which are added to a basket. When the user decides to place the books on the shelf, it takes each book from the basket and places it on the shelf using the first letter of the name to identify the correct slot. It also provides an option to display the contents of the library and exit the library manager.

Since the design establishes unambiguous semantic rules for it to be realized faithfully, a failure to respect these intentions results in predictable failure modalities that can be resolved by realigning the behavior of the design with its intentions. 
