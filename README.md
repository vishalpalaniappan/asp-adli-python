...
To inject logs into a single python program, create a metadata file and follow the usage instructions.

### Metadata File

A metadata file named adli_metadata.json must be included in the source file directory and this information will be included in the header of the CDL file. In the future, this file can also be used to configure the ADLI tool.
...
After creating an `adli_metadata.json` file in the source file directory, run the tool using the following command (using Python 3.9+):
  ...
  ```shell
  python adli.py -source <source_file>
  ```
